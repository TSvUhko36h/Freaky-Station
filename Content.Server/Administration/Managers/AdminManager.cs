...
using Content.Server.Discord; //ADT port
using Serilog; //ADT port
...
namespace Content.Server.Administration.Managers
{
    public sealed partial class AdminManager : IAdminManager, IPostInjectInit, IConGroupControllerImplementation
    {
        ...
        {
            // Вызываем асинхронную версию и игнорируем Task
            _ = DeAdminAsync(session);
        }

        private async Task DeAdminAsync(ICommonSession session)
        {
        ...
        {
            // Вызываем асинхронную версию и игнорируем Task
            _ = ReAdminAsync(session);
        }

        private async Task ReAdminAsync(ICommonSession session)
        {
            if (!_admins.TryGetValue(session, out var reg))
            {
                throw new ArgumentException($"Player {session} is not an admin");
